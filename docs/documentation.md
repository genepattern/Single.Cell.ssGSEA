...
6. `cluster_data_file`  
    If your input file is `h5` or `10x MEX` format, a separate cluster data file (tab-delimited .txt file) must be supplied here. The grouping information in this file is used to aggregate cells prior to computing scGSEA scores. Therefore, if you have `h5` or `10x MEX` formatted files and do not have cluster data file, please use `Scanpy Utilities` module or the GenePattern Seurat Suite (`Seurat.QC` > `Seurat.Preprocess` > `Seurat.Clustering`) to compute clusters.
